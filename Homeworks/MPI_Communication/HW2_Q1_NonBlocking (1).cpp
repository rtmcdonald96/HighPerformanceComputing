...
int main( int argc, char *argv[] )
{
  ...
  if (rank == 1) {
    ...
    MPI_Isend(&value, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &request);
    MPI_Wait(&request, &status);
  //Process rank 0 recieves 10.1 from process rank 1
} else if (rank == 0) {
    MPI_Irecv(&value, 1, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, &request);
    MPI_Wait(&request, &status);
  ...
  if(rank == 1)
  {
  printf("Total execution time for single send(non-blocking): %f\n", end_time-start_time);
  }
  if(rank == 0)
  {
  printf("Total execution time for round trip(non-blocking): %f\n", round_time-start_time);
  }
  MPI_Finalize();
